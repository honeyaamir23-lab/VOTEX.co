...
export default function AdminPanel() {
  ...
  // Calculate metrics
  const totalVotes = state?.battles?.reduce((acc, b) => acc + b.player1Votes + b.player2Votes, 0) || 0;
  const totalDepositsValue = state?.deposits?.filter(d => d.status === 'APPROVED').reduce((sum, d) => sum + d.amount, 0) || 0;
  const totalWithdrawalsValue = state?.pendingRequests?.filter(r => r.type === 'WITHDRAWAL' && r.status === 'PAID').reduce((sum, r) => sum + Number(r.amount), 0) || 0;
  const totalCoinsValue = state?.users?.reduce((sum, u) => sum + u.balance, 0) || 0;
  const totalBattleRevenue = state?.battles?.filter(b => b.status === 'COMPLETED').reduce((sum, b) => sum + b.pot, 0) || 0;
  const totalPaidVoteRevenue = state?.transactions?.filter(t => t.type.startsWith('PAID_VOTE_')).reduce((sum, t) => sum + Math.abs(t.amount), 0) || 0;
  
  const pendingDepositsList = state?.deposits?.filter(d => d.status === 'PENDING') || [];
    ...
    <div className="flex flex-col sm:flex-row h-full w-full bg-[#050505] overflow-hidden">
      ...
      <div className="w-full sm:w-64 bg-[#0A0D14] border-b sm:border-b-0 sm:border-r border-gray-800 p-4 flex sm:flex-col overflow-x-auto sm:overflow-y-auto sm:h-full shrink-0">
        <div className="hidden sm:block mb-8 px-2">
          <h2 className="text-xl font-bold tracking-tight text-white mb-1">Admin Dashboard</h2>
          ...
          {tabs.map((tab, idx) => (
            <button
              ...
              className={`px-4 py-3 rounded-xl text-sm font-bold whitespace-nowrap transition-colors flex items-center gap-2 ${
                activeTab === tab ? 'bg-[#10B981] text-black shadow-[0_0_15px_rgba(16,185,129,0.3)]' : 'text-gray-400 hover:text-white hover:bg-gray-800'
              }`}
            >
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-8 bg-[#050505] pb-24 sm:pb-8">
        <div className="max-w-6xl mx-auto space-y-6">
          <div className="mb-6 flex justify-between items-center hidden sm:flex">
            <h2 className="text-2xl font-bold text-white tracking-tight">{activeTab === 'Stats' ? 'Platform Overview' : activeTab}</h2>
            {activeTab === 'Users' && (
            ...
            {activeTab !== 'Users' && (
               <div className="text-sm text-gray-500 font-mono">Live Sync Active</div>
            )}
          </div>
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="bg-[#131823] border border-gray-800 rounded-2xl p-4 lg:p-6 shadow-lg">
                  ...
                  <div className="text-3xl font-black text-white">{stats.activeBattles}</div>
                </div>
                ...
                <div className="bg-[#131823] border border-gray-800 rounded-2xl p-4 lg:p-6 shadow-lg">
                  ...
                  <div className="text-3xl font-black text-white">{stats.pendingDeposits}</div>
                </div>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                 <div className="bg-gradient-to-br from-[#131823] to-[#0A0D14] border border-gray-800 rounded-2xl p-4 lg:p-6 shadow-lg">
                    <div className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-2">Total Deposits (VTX)</div>
                    <div className="text-2xl font-black text-emerald-400">{totalDepositsValue.toLocaleString()}</div>
                 </div>
                 <div className="bg-gradient-to-br from-[#131823] to-[#0A0D14] border border-gray-800 rounded-2xl p-4 lg:p-6 shadow-lg">
                    <div className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-2">Total Paid Vote Revenue</div>
                    <div className="text-2xl font-black text-purple-400">{totalPaidVoteRevenue.toLocaleString()} VTX</div>
                 </div>
                 <div className="bg-gradient-to-br from-[#131823] to-[#0A0D14] border border-gray-800 rounded-2xl p-4 lg:p-6 shadow-lg">
                    <div className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-2">Total Withdrawals</div>
                    <div className="text-2xl font-black text-[#F43F5E]">{totalWithdrawalsValue.toLocaleString()} VTX</div>
                 </div>
                 <div className="bg-gradient-to-br from-[#131823] to-[#0A0D14] border border-gray-800 rounded-2xl p-4 lg:p-6 shadow-lg">
                    <div className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-2">Commission ({stats.platformFeePercent}%)</div>
                    <div className="text-2xl font-black text-yellow-400">{stats.totalCommission.toLocaleString()} VTX</div>
                 </div>
              </div>
               ...
               <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div className="bg-[#131823] border border-gray-800 rounded-2xl p-5 shadow-lg flex flex-col h-80">
                     <h3 className="font-bold text-white mb-4 border-b border-gray-800 pb-2">New Registrations</h3>
                     <div className="flex-1 overflow-y-auto pr-2 space-y-3 custom-scrollbar">
                        {state?.users?.filter(u => !u.is_bot).sort((a,b) => (b.createdAt || 0) - (a.createdAt || 0)).slice(0, 10).map(u => (
                           <div key={u.id} className="flex justify-between items-center bg-[#0A0D14] p-3 rounded-xl border border-gray-800/50">
                               <div className="flex items-center gap-3">
          ...
          {activeTab === 'Deposits' && (
            <div className="space-y-3 animate-in fade-in slide-in-from-bottom-2 duration-300">
               {pendingDepositsList.length > 0 ? pendingDepositsList.map((req) => {
                   const user = state?.users.find(u => u.id === req.userId);
                   return (
                      <div key={req.id} className="bg-[#131823] border border-gray-800 rounded-xl p-4 flex flex-col gap-3 shadow-lg">
                          <div className="flex items-center justify-between">
                              <div>
                                 <p className="font-bold text-sm text-white">@{user?.username || 'Unknown'} <span className="text-gray-400 text-xs ml-1">(ID: {user?.uniqueId || 'N/A'})</span></p>
                                 <p className="text-xs text-gray-500">{new Date(req.timestamp).toLocaleString()}</p>
                              </div>
                              <div className="text-emerald-400 font-mono font-bold">+{req.amount}</div>
                          </div>
                             ...
                             {req.transactionId && (
                               <div className="bg-gray-800/50 text-gray-300 py-2 px-3 rounded-lg text-xs font-mono border border-gray-700 flex flex-col gap-1">
                                 {req.transactionId.includes('::') ? (
                                    <>
                                      <span className="text-gray-400 font-sans font-bold text-[10px] uppercase tracking-wider">{req.transactionId.split('::')[0].trim()}</span>
                                      <span>TID: <span className="text-[#10B981] font-bold">{req.transactionId.split('::')[1].trim()}</span></span>
                                    </>
                                 ) : (
                                    <span>TID: <span className="text-[#10B981] font-bold">{req.transactionId}</span></span>
                                 )}
                               </div>
                             )}
                             <div className="flex items-center gap-2">
                               ...
                               <button 
                                  onClick={() => adminAction('REJECT_DEPOSIT', { requestId: req.id })}
                               ...
                               <button 
                                  onClick={() => adminAction('APPROVE_DEPOSIT', { requestId: req.id, amount: req.amount })}
          ...
          {activeTab === 'Withdrawals' && (
            <div className="space-y-3 animate-in fade-in slide-in-from-bottom-2 duration-300">
               {pendingWithdrawalsList.length > 0 ? pendingWithdrawalsList.map((req) => {
                   const user = state?.users.find(u => u.id === req.userId);
                   return (
                      <div key={req.id} className="bg-[#131823] border border-gray-800 rounded-xl p-4 flex items-center justify-between shadow-lg">
                          <div>
                             <p className="font-bold text-sm text-white">@{user?.username || 'Unknown'} <span className="text-gray-400 text-xs ml-1">(ID: {user?.uniqueId || 'N/A'})</span></p>
                             <p className="text-xs text-gray-500 mb-1">{new Date(req.date).toLocaleString()}</p>
                             <p className="text-sm font-mono text-[#10B981]">{req.easypaisaNumber}</p>
                          </div>
                          <div className="flex items-center gap-4">
                             <div className="text-[#F43F5E] font-mono font-bold">-{req.amount}</div>
                             <button 
                                onClick={() => adminAction('APPROVE_WITHDRAWAL', { requestId: req.id })}
                                className="bg-emerald-500/20 text-emerald-400 p-2 rounded-lg hover:bg-emerald-500/40 transition-colors"
                             >
                                <Check className="w-5 h-5" />
                             </button>
                             <button 
                                onClick={() => adminAction('REJECT_WITHDRAWAL', { requestId: req.id })}
          ...
          {activeTab === 'Battles' && (
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                 ...
                 <div className="bg-[#131823] p-5 rounded-xl border border-gray-800 shadow-lg h-fit">
                    <h3 className="text-white font-bold mb-4">Create Display Battle (Bots)</h3>
                    <form onSubmit={(e) => {
                       e.preventDefault();
                       ...
                       const type = formData.get('battleType') as string || 'QUICK';
                       const p1Votes = Number(formData.get('p1Votes')) || 0;
                       const p2Votes = Number(formData.get('p2Votes')) || 0;
                       if (p1 && p2 && p1 !== p2 && stake > 0) {
                          adminAction('CREATE_ADMIN_BATTLE', { player1Id: p1, player2Id: p2, stake, type, p1Votes, p2Votes });
                       ...
                       <div className="grid grid-cols-2 gap-4">
                          <div>
                             <label className="block text-xs font-bold text-gray-400 mb-2">Player 1</label>
                             <select name="player1" className="w-full bg-[#1A1F2E] border border-gray-800 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-[#10B981]">
                          ...
                          <div>
                             <label className="block text-xs font-bold text-gray-400 mb-2">Player 2</label>
                             <select name="player2" className="w-full bg-[#1A1F2E] border border-gray-800 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-[#10B981]">
                                <option value="">Select Bot...</option>
                                {state?.users?.filter(u => u.is_bot).map(bot => (
                                   <option key={bot.id} value={bot.id}>@{bot.username}</option>
                                ))}
                       ...
                       <div className="grid grid-cols-2 gap-4">
                          <div>
                             <label className="block text-xs font-bold text-gray-400 mb-2">Battle Type</label>
                             <select name="battleType" className="w-full bg-[#1A1F2E] border border-gray-800 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-[#10B981]">
                                <option value="QUICK">Quick Battle (2 minutes)</option>
                                <option value="GRAND">Grand Battle (24 hours)</option>
                             </select>
                          </div>
                          <div>
                             <label className="block text-xs font-bold text-gray-400 mb-2">Virtual Stake</label>
                             <input 
                                name="stake"
                                type="number"
                                min="100"
                       ...
                       <div className="grid grid-cols-2 gap-4">
                          <div>
                             <label className="block text-xs font-bold text-gray-400 mb-2">P1 Initial Votes</label>
                             <input 
                                name="p1Votes"
                                type="number"
                                min="0"
                          ...
                          <div>
                             <label className="block text-xs font-bold text-gray-400 mb-2">P2 Initial Votes</label>
                             <input 
                                name="p2Votes"
                                type="number"
                                min="0"
                       ...
                       <button type="submit" className="w-full bg-[#10B981] text-black font-bold px-4 py-3 rounded-lg hover:bg-[#059669]">
                          Start Battle
                       </button>
                    </form>
                 </div>
                 ...
                 <div className="bg-[#131823] p-5 rounded-xl border border-gray-800 shadow-lg flex flex-col h-[600px]">
                    <h3 className="text-white font-bold mb-4 border-b border-gray-800 pb-2">Active Battles Overview</h3>
                    <div className="flex-1 overflow-y-auto space-y-3 custom-scrollbar pr-2">
                    {state?.battles?.filter(b => b.status === 'LIVE' || b.status === 'WAITING').map(battle => {
                       const p1 = state.users.find(u => u.id === battle.player1Id);
                       const p2 = state.users.find(u => u.id === battle.player2Id);
                       return (
                          <div key={battle.id} className="bg-[#0A0D14] border border-gray-800/50 rounded-xl p-4 flex items-center justify-between">
                             <div>
                                <p className="font-bold text-sm text-white">@{p1?.username || 'Unknown'} <span className="text-gray-500 mx-2">vs</span> @{p2?.username || 'Pending'}</p>
                                <p className="text-xs text-emerald-400 mt-1 font-mono">Pot: {battle.pot} VTX</p>
                             </div>
                             <button 
                                onClick={() => {
                                   if (window.confirm('Cancel this battle?')) {
                                      adminAction('CANCEL_BATTLE', { battleId: battle.id });
                                   }
                                }}
                    ...
                    {state?.battles?.filter(b => b.status === 'LIVE' || b.status === 'WAITING').length === 0 && (
                       <p className="text-gray-500 text-sm text-center py-10">No active battles.</p>
                    )}
                    </div>
                 </div>
                 </div>
          ...
          {activeTab === 'Users' && (
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                 ...
                 <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                     <div className="bg-[#131823] p-5 rounded-xl border border-gray-800 shadow-lg h-fit">
                        <h3 className="text-white font-bold mb-4">Create Display Bot</h3>
                        <form onSubmit={(e) => {
                           e.preventDefault();
                           const formData = new FormData(e.currentTarget);
                           ...
                           />
                           <button type="submit" className="bg-[#10B981] text-black font-bold px-4 py-2 rounded-lg hover:bg-[#059669]">
                              Add Bot
                           </button>
                        </form>
                     </div>
                     ...
                     <div className="bg-[#131823] p-5 rounded-xl border border-gray-800 shadow-lg flex flex-col h-[400px]">
                        ...
                        {state?.users?.filter(u => u.is_bot).length === 0 && (
                           <p className="text-gray-500 text-sm text-center py-4">No bots created yet.</p>
                        )}
                        </div>
                     </div>
                 </div>
          ...
          {activeTab === 'Support' && (
             <div className="space-y-6 animate-fade-in">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                     {state?.pendingRequests?.filter(r => r.type === 'SUPPORT_TICKET').sort((a, b) => b.date - a.date).map(ticket => {
                         const user = state?.users?.find(u => u.id === ticket.userId);
                         return (
                             <div key={ticket.id} className="bg-[#131823] border border-gray-800 rounded-2xl p-4 shadow-lg">
                                 <div className="flex justify-between items-start mb-2">
                                     <div>
                                         <p className="font-bold text-white text-sm">@{user?.username || 'Unknown'} <span className="text-xs text-gray-500 font-mono ml-1">({user?.uniqueId})</span></p>
                                         <p className="text-xs text-gray-500">{new Date(ticket.date).toLocaleString()}</p>
                                     </div>
                                     <span className={`text-[10px] uppercase font-black tracking-wider px-2 py-1 rounded-lg ${
                                         ticket.status === 'PENDING' ? 'bg-yellow-500/20 text-yellow-500' : 'bg-emerald-500/20 text-emerald-400'
                                     }`}>
                                         {ticket.status}
                                     </span>
                                 </div>
                                 ...
                                 {ticket.status === 'PENDING' && (
                                     <button 
                                         onClick={() => adminAction('RESOLVE_TICKET', { ticketId: ticket.id })}
                     ...
                     {(!state?.pendingRequests?.filter(r => r.type === 'SUPPORT_TICKET').length) && (
          ...
          {activeTab === 'Settings' && (
             <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
                <div className="bg-[#131823] p-5 lg:p-8 rounded-2xl border border-gray-800 shadow-lg">
                   ...
                   <form key={JSON.stringify(stats)} 
                      onSubmit={(e) => {
                         ...
                         adminAction('UPDATE_SETTINGS', {
                            ...
                            platformFeePercent: Number(formData.get('platformFeePercent')),
                            ...
                            enableQuickBattles: formData.get('enableQuickBattles') === 'on',
                            enableGrandBattles: formData.get('enableGrandBattles') === 'on'
                         });
                   ...
                   >
                      <div>
                         <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Signup Bonus (VTX)</label>
                         <input 
                            name="signupBonus"
                      ...
                      <div className="col-span-1 md:col-span-2 mt-2 p-4 bg-black/20 rounded-xl border border-gray-800">
                           <h4 className="text-[#10B981] font-bold border-b border-[#10B981]/20 pb-2 mb-4">Easypaisa Configuration</h4>
                           <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                               <div>
                                  <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Account IBAN</label>
                                  <input 
                                     name="epIban"
                                     defaultValue={stats.paymentConfig?.easypaisa?.iban || ''}
                                     type="text" 
                               ...
                               <div>
                                  <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Deep Link</label>
                                  <input 
                                     name="epLink"
                                     defaultValue={stats.paymentConfig?.easypaisa?.deepLink || ''}
                                     type="url" 
                               ...
                               <div>
                                  ...
                                  <input 
                                     name="epQr"
                                     defaultValue={stats.paymentConfig?.easypaisa?.qrUrl || ''}
                                     type="url" 
                                     placeholder="https://..."
                                     className="w-full bg-[#1A1F2E] border border-gray-800 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#10B981] transition-colors"
                                  />
                       ...
                       <div className="col-span-1 md:col-span-2 mb-2 p-4 bg-black/20 rounded-xl border border-gray-800">
                           <h4 className="text-blue-400 font-bold border-b border-blue-400/20 pb-2 mb-4">SadaPay Configuration</h4>
                           <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                               <div>
                                  <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Account IBAN</label>
                                  <input 
                                     name="spIban"
                                     defaultValue={stats.paymentConfig?.sadapay?.iban || ''}
                                     type="text" 
                               ...
                               <div>
                                  ...
                                  <input 
                                     name="spLink"
                                     defaultValue={stats.paymentConfig?.sadapay?.deepLink || ''}
                                     type="url" 
                                     placeholder="sadapay://"
                                     className="w-full bg-[#1A1F2E] border border-gray-800 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-blue-400 transition-colors"
                                  />
                               ...
                               <div>
                                  ...
                                  <input 
                                     name="spQr"
                                     defaultValue={stats.paymentConfig?.sadapay?.qrUrl || ''}
                                     type="url" 
                                     placeholder="https://..."
                                     className="w-full bg-[#1A1F2E] border border-gray-800 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-blue-400 transition-colors"
                                  />
                      ...
                      <div>
                         <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Min Withdrawal (VTX)</label>
                         <input 
                            name="minWithdrawal"
                      ...
                      <div>
                         <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Max Withdrawal (VTX)</label>
                         <input 
                            name="maxWithdrawal"
                      ...
                      <div>
                         <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Quick Battle Stakes (CSV)</label>
                         <input 
                            name="quickStakes"
                      ...
                      <div className="md:col-span-2">
                         <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Grand Battle Stakes (CSV)</label>
                         <input 
                            name="grandStakes"
                            defaultValue={stats.grandStakes || '10000,25000,50000,100000'}
                            type="text" 
                            className="w-full bg-[#1A1F2E] border border-gray-800 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#10B981] transition-colors"
                         />
                      </div>
                      <div className="flex gap-6 md:col-span-2 p-4 bg-[#0A0D14] border border-gray-800 rounded-xl">
                         <label className="flex items-center gap-3 cursor-pointer text-white">
                            <input type="checkbox" name="enableQuickBattles" defaultChecked={stats.enableQuickBattles !== false} className="w-5 h-5 accent-[#10B981]" />
                            <span className="font-bold text-sm">Enable Quick Battles</span>
                         </label>
                         <label className="flex items-center gap-3 cursor-pointer text-white">
                            <input type="checkbox" name="enableGrandBattles" defaultChecked={stats.enableGrandBattles !== false} className="w-5 h-5 accent-[#10B981]" />
                            <span className="font-bold text-sm">Enable Grand Battles</span>
                         </label>
                      </div>
                      <div className="md:col-span-2 pt-4">
                         <button type="submit" className="w-full md:w-auto px-8 bg-[#10B981] text-black font-bold py-3 rounded-xl hover:bg-[#0ea5e9] transition-colors shadow-[0_0_20px_rgba(16,185,129,0.3)]">
                            Save Settings
                         </button>
                      </div>
                   </form>
          ...
          {selectedReceipt && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm animate-in fade-in" onClick={() => setSelectedReceipt(null)}>
               <div className="relative max-w-4xl w-full" onClick={e => e.stopPropagation()}>
                  ...
                  <img src={selectedReceipt} alt="Receipt" className="w-full h-auto max-h-[85vh] object-contain rounded-2xl border border-gray-800" />
               </div>
            </div>
          )}
        </div>
