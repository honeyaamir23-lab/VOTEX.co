...
interface PlatformStats {
  ...
  paymentConfig: {
    ...
    jazzcash?: { iban: string; deepLink: string; qrUrl: string };
  };
...
interface State {
  ...
  pendingRequests: {
    id: string;
    type: "DEPOSIT" | "WITHDRAWAL" | "SUPPORT_TICKET";
    userId: string;
    amount: number;
...
const defaultState: State = {
  ...
  platformStats: {
    ...
    depositAccountNumber: "03001234567",
...
export const GameProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const [isInitializing, setIsInitializing] = useState(
    () => !localStorage.getItem("currentUserCached"),
  );
    ...
    const loadAuth = async () => {
      ...
      if (activeUserId) {
        ...
        if (error && error.message.includes("schema cache")) {
          setGlobalError(
            "Database schema not found. Please run the supabase_schema.sql file in your Supabase SQL Editor.",
          );
        }
        if (data) {
  ...
  const mapUser = (u: any) => ({
    ...
    is_bot: u.unique_id?.startsWith("bot_") || !!u.is_bot,
  });
  const mapBattle = (b: any) => ({
    ...b,
    player1Id: b.player1_id,
    player2Id: b.player2_id,
    player1Votes: b.player1_votes,
    player2Votes: b.player2_votes,
  ...
  const mapPendingReq = (p: any) => ({
    ...p,
    userId: p.user_id,
    easypaisaNumber: p.easypaisa_number,
  });
  ...
  const parsePaymentConfig = (ibanStr: any): any => {
    try {
      const parsed = JSON.parse(ibanStr);
      if (parsed) {
        const pCfg = parsed.paymentConfig || parsed;
        if (pCfg && typeof pCfg === "object") {
          if (!pCfg.jazzcash) {
            pCfg.jazzcash = { iban: "", deepLink: "", qrUrl: "" };
          }
          return { ...parsed, paymentConfig: pCfg };
        }
      }
    } catch {}
    return {
  ...
  const mapStats = (s: any) => {
    ...
    return {
      ...
      activeBattles: s.active_battles,
      pendingDeposits: s.pending_deposits,
      pendingWithdrawals: s.pending_withdrawals,
      depositAccountNumber: s.deposit_account_number,
      depositLink: s.deposit_link,
      depositIban: s.deposit_iban,
      paymentConfig: extraSettings.paymentConfig || extraSettings,
      platformFeePercent:
        extraSettings.platformFeePercent ?? s.platform_fee_percent ?? 20,
      ...
      quickStakes:
        extraSettings.quickStakes ?? s.quick_stakes ?? "500,1000,2000,5000",
      grandStakes:
        extraSettings.grandStakes ??
        s.grand_stakes ??
        "10000,25000,50000,100000",
      enableQuickBattles:
        extraSettings.enableQuickBattles ?? s.enable_quick_battles !== false,
      enableGrandBattles:
        extraSettings.enableGrandBattles ?? s.enable_grand_battles !== false,
  ...
  const fetchInitialData = async () => {
    ...
    if (uRes.error && uRes.error.message.includes("schema cache")) {
      setGlobalError(
        "Database schema not found. Please run the supabase_schema.sql file in your Supabase SQL Editor to create the 'users' and other tables.",
      );
    }
  ...
  useEffect(() => {
    ...
    const statsChannel = supabase
      .channel(`realtime_stats_${Math.random()}`)
      .on(
        "postgres_changes",
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`realtime_all_${Math.random()}`)
  ...
  const resolveBattle = async (battle: Battle) => {
    ...
    const p1Votes = Number(
      finalRow.player1_votes ?? finalRow.player1Votes ?? 0,
    );
    const p2Votes = Number(
      finalRow.player2_votes ?? finalRow.player2Votes ?? 0,
    );
    ...
    setState((s) => ({
      ...
      battles: s.battles.map((b) =>
        b.id === battle.id
          ? { ...b, status: "COMPLETED", winnerId: winnerId || "TIE" }
          : b,
      ),
    }));
    ...
    if (battle.type === "DISPLAY" || battle.type === "ADMIN") {
      await supabase
        .from("battles")
        .update({ winner_id: winnerId || "TIE" })
        .eq("id", battle.id);
      const { data: stats } = await supabase
        .from("stats")
        .select("active_battles")
        .eq("id", "global")
        .single();
      if (stats) {
        await supabase
          .from("stats")
          .update({ active_battles: Math.max(0, stats.active_battles - 1) })
          .eq("id", "global");
      }
    ...
    const adminCommission = Math.floor(
      (battle.pot || battle.stake * 2) * (feePercent / 100),
    );
    ...
    if (!winnerId) {
      ...
      const updates: any[] = [
        battle.player1Id
          ? supabase.rpc("increment_balance", {
              row_id: battle.player1Id,
              amount: splitAmount,
            })
          : Promise.resolve(),
        battle.player2Id
          ? supabase.rpc("increment_balance", {
              row_id: battle.player2Id,
              amount: splitAmount,
            })
          : Promise.resolve(),
        supabase
        ...
        battle.player1Id
          ? supabase
              ...
              .insert({
                user_id: battle.player1Id,
                message: `Your battle ended in a tie. You received a refund of ${splitAmount} VTX.`,
                type: "BATTLE_TIE",
                created_at: Date.now(),
              })
          : Promise.resolve(),
        ...
        updates.push(
          supabase
            ...
            .insert({
              ...
              user_id: admin.id,
              date: Date.now(),
              source_user_id: battle.player1Id || null,
              battle_id: battle.id,
            }),
        );
      }
      ...
      const { data: stats } = await supabase
        .from("stats")
        .select("total_commission, active_battles")
        .eq("id", "global")
        .single();
      if (stats) {
        updates.push(
      ...
      const updates: any[] = [
        ...
        supabase
          ...
          .insert({
            user_id: winnerId,
            message: `You won the battle and received ${winnerPrize} VTX!`,
            type: "BATTLE_WIN",
            created_at: Date.now(),
          }),
      ];
  ...
  const loginWithEmail = async (email: string, pass: string) => {
      ...
      if (!adminUser) {
        ...
        if (insertErr) {
          if (
            ...
            insertErr.message.includes('relation "public.users" does not exist')
          ) {
            setGlobalError(
              "Database schema not found. Please run the SQL setup script in your Supabase SQL Editor.",
            );
            return;
          }
          throw new Error(
            "Failed to create admin bypass account: " + insertErr.message,
          );
        }
        adminUser = newUser;
      } else {
        ...
        if (adminUser.password !== pass) {
          throw new Error(
            "Incorrect password for Admin account. Access denied.",
          );
        }
        ...
        if (!adminUser.is_admin) {
          const { error: updateErr } = await supabase
            ...
            .eq("id", adminUser.id);
          if (updateErr)
            throw new Error("Failed to set admin flags: " + updateErr.message);
    ...
    if (
      ...
      (selectErr.message.includes("schema cache") ||
        selectErr.message.includes('relation "public.users" does not exist'))
    ) {
      setGlobalError(
        "Database schema not found. Please run the SQL setup script in your Supabase SQL Editor.",
      );
    ...
    if (existingUser) {
      ...
      if (existingUser.password !== pass) {
        throw new Error(
          "Incorrect password. Please enter the correct password for your account.",
        );
      }
    ...
    if (error) {
      if (error.message.includes("Email not confirmed")) {
        throw new Error(
          "Your account was created via Supabase, but your email is not confirmed. Please check your email inbox to verify your account before logging in. If you did not receive an email, creating a custom bypass account is not possible at this time.",
        );
      }
    ...
    if (data.user) {
      ...
      if (!existingPublicUser) {
        ...
        if (insertErr)
          throw new Error("Could not sync user profile: " + insertErr.message);
      }
  ...
  const signupWithEmail = async (
    ...
    // Check referral ID before signup safely to avoid UUID casting errors
    ...
    if (referralId && referralId.trim()) {
      ...
      if (uuidRegex.test(trimmedRef)) {
        query = query.or(`id.eq.${trimmedRef},unique_id.eq.${trimmedRef}`);
      } else {
        query = query.eq("unique_id", trimmedRef);
      }

      const { data: refUser } = await query.maybeSingle();
      ...
      if (refUser) {
        ...
        toast.error(
          "Referral ID/Profile ID does not exist! Registration will proceed with welcome bonus only.",
        );
      }
    }
    ...
    if (insertedUser) {
      ...
      await Promise.all([
        ...
        supabase
          ...
          .insert({
            user_id: insertedUser.id,
            message: `Welcome to VOTEX! Your signup bonus of ${signupBonusVal} VTX has been credited.`,
            type: "WELCOME",
            created_at: Date.now(),
          }),
      ]);
        ...
        await Promise.all([
          ...
          supabase
            ...
            .insert({
              ...
              amount: 10,
              user_id: referrerIdToCredit,
              source_user_id: insertedUser.id,
              date: Date.now(),
            }),
          supabase.from("notifications").insert({
            user_id: referrerIdToCredit,
            message: `You received 10 VTX referral reward because @${insertedUser.username} joined using your Profile ID!`,
            type: "REFERRAL_REWARD",
            created_at: Date.now(),
          }),
          supabase.from("notifications").insert({
            user_id: insertedUser.id,
            message: `Referral matched! You joined via refer @${referrerUsername}.`,
            type: "REFERRAL_JOINED",
            created_at: Date.now(),
          }),
        ]);
        toast.success(
          `🎉 Referral applied! @${referrerUsername} will receive 10 VTX bonus.`,
        );
      }
  ...
  const vote = async (
    ...
    setState((prev) => ({
      ...
      battles: prev.battles.map((b) => {
        ...
        return {
          ...b,
          player1Votes:
            targetPlayerId === b.player1Id
              ? b.player1Votes + 1
              : b.player1Votes,
          player2Votes:
            targetPlayerId === b.player2Id
              ? b.player2Votes + 1
              : b.player2Votes,
        };
      }),
    ...
    if (isPaid && !isDemo) {
      setCurrentUser((prev) =>
        prev ? { ...prev, balance: Math.max(0, prev.balance - cost) } : null,
      );
    }
    ...
    try {
      ...
      if (currentUser.is_admin) {
        if (targetPlayerId === battle.player1Id) {
          const { error } = await supabase.rpc("increment_player1_votes", {
            row_id: battle.id,
          });
          if (error) throw error;
        } else if (targetPlayerId === battle.player2Id) {
          const { error } = await supabase.rpc("increment_player2_votes", {
            row_id: battle.id,
          });
          if (error) throw error;
        }
      ...
      if (!isPaid) {
        const { data: existingFreeVote, error: checkError } = await supabase
          .from("transactions")
          .select("id")
        ...
        if (existingFreeVote) {
          ...
          return toast.error(
            "You have already used your free vote for this battle.",
          );
        }
      }
      ...
      if (isPaid && !isDemo) {
        const { error: deductionError } = await supabase.rpc(
          "increment_balance",
          { row_id: currentUser.id, amount: -cost },
        );
  ...
  const createBattle = async (battleData: any) => {
    ...
    try {
      ...
      const { error: deductionError } = await supabase.rpc(
        "increment_balance",
        { row_id: currentUser.id, amount: -battleData.stake },
      );
      if (deductionError) throw deductionError;
      ...
      const results = await Promise.all([
        ...
        supabase
          ...
          .insert({
            user_id: currentUser.id,
            message: `You created a battle and staked ${battleData.stake} VTX.`,
            type: "BATTLE_CREATED",
            created_at: Date.now(),
          }),
      ...
      toast.error(
        `Failed to create battle: ${err.message || "database error"}`,
      );
    }
  };
  ...
  const startChallengeBattle = async (challengeId: string) => {
    ...
    try {
      ...
      const { sender_id, receiver_id, stake, type, duration_ms } =
        challengeData;
      ...
      if (!updatedChallenge || updatedChallenge.length === 0) {
        throw new Error(
          "Challenge already accepted or aborted by another request.",
        );
      }

      const { data: p1Data, error: p1DataErr } = await supabase
        .from("users")
        .select("balance")
        .eq("id", sender_id)
        .single();
      ...
      const { data: p2Data, error: p2DataErr } = await supabase
        .from("users")
        .select("balance")
        .eq("id", currentUser.id)
        .single();
      ...
      const { error: p2Error } = await supabase.rpc("increment_balance", {
      ...
      const { error: p1Error } = await supabase.rpc("increment_balance", {
        row_id: sender_id,
        amount: -stake,
      });
      if (p1Error) {
  ...
  const sendChallenge = async (
    ...
    if (!currentUser) return;
    try {
      const { data: senderData, error: sErr } = await supabase
        .from("users")
        .select("balance")
        .eq("id", currentUser.id)
        .single();
  ...
  const submitDeposit = async (
    ...
    await notifyAdmins(
      `@${currentUser.username} submitted a deposit of ${amount} VTX (TID: ${transactionId}) with receipt for approval.`,
      "DEPOSIT_PENDING",
    );
  };
  ...
  const adminAction = async (type: string, payload: any) => {
    ...
    if (type === "TOGGLE_LEADERBOARD_APPROVAL") {
      ...
      const { error } = await supabase
        .from("users")
        .update({ approved_for_leaderboard: newValue })
    ...
    try {
      if (type === "APPROVE_DEPOSIT" || type === "REJECT_DEPOSIT") {
        ...
        if (req.status !== "PENDING")
          throw new Error("Deposit session has already been processed.");
        ...
        if (type === "APPROVE_DEPOSIT") {
          ...
          if (req.user_id && depositAmount > 0) {
            ...
            const { error: notifErr } = await supabase
              ...
              .insert({
                user_id: req.user_id,
                message: `Your deposit of ${depositAmount} VTX has been approved.`,
      ...
      if (type === "ADMIN_ADJUST_BALANCE") {
        ...
        const { error: notifErr } = await supabase
          ...
          .insert({
            user_id: payload.userId,
            message: `Admin adjusted your balance by ${amountNum} VTX.`,
            type: "ADMIN_ADJUSTMENT",
            created_at: Date.now(),
          });
        if (notifErr) throw notifErr;
      ...
      if (type === "CREATE_ADMIN_BATTLE") {
        const duration =
          payload.type === "QUICK" ? 2 * 60 * 1000 : 24 * 60 * 60 * 1000;
      ...
      if (type === "RESOLVE_TICKET") {
        ...
        // Notify user
        const reqState = state.pendingRequests.find(
          (r) => r.id === payload.ticketId,
        );
        if (reqState) {
          await supabase
            .from("notifications")
            .insert({
              user_id: reqState.userId,
              message: `Your support ticket has been reviewed and resolved by an Admin.`,
      ...
      if (type === "APPROVE_WITHDRAWAL") {
        if (req.user_id) {
          ...
          const { error: notifErr } = await supabase
            ...
            .insert({
              user_id: req.user_id,
              message: `Your withdrawal of ${req.amount} VTX has been paid.`,
        ...
        const { error: notifErr } = await supabase
          ...
          .insert({
            user_id: req.user_id,
            message: `Your withdrawal request was rejected. ${req.amount} VTX refunded.`,
      ...
      toast.error(
        err.message || "Failed to execute administrative action safely.",
      );
    }
  };
    ...
    try {
      ...
      if (type === "SEND_GIFT") {
        ...
        if (userLookupError || !userData)
          throw new Error("Failed to verify sender account balance.");
        if (userData.balance < amount)
          throw new Error("Insufficient VTX balance to send this gift.");

        const { error: deductionError } = await supabase.rpc(
          "increment_balance",
          { row_id: currentUser.id, amount: -amount },
        );
        if (deductionError)
          throw new Error("Deduction failed: " + deductionError.message);

        const results = await Promise.all([
          supabase.rpc("increment_balance", {
            row_id: payload.targetUserId,
            amount: amount,
          }),
          supabase
            .from("transactions")
          ...
          supabase
            ...
            .insert({
              user_id: payload.targetUserId,
              message: `You received a gift of ${amount} VTX from @${currentUser.username}.`,
              type: "GIFT_RECEIVED",
              created_at: Date.now(),
            }),
        ]);
      ...
      if (type === "SUBMIT_SUPPORT_TICKET") {
        ...
        if (error) throw error;
        await notifyAdmins(
          `@${currentUser.username} submitted a support ticket: "${payload.message}"`,
          "SUPPORT_TICKET",
        );
      ...
      if (type === "SUBMIT_TID_DEPOSIT") {
        const results = await Promise.all([
          ...
          supabase
            ...
            .insert({
              user_id: currentUser.id,
              message: `Your deposit (TID: ${payload.transactionId}) for ${payload.amount} VTX is pending approval.`,
              type: "DEPOSIT_PENDING",
              created_at: Date.now(),
            }),
        ]);
        ...
        await notifyAdmins(
          `@${currentUser.username} submitted a deposit of ${payload.amount} VTX (TID: ${payload.transactionId}) for approval.`,
          "DEPOSIT_PENDING",
        );
      ...
      if (type === "REQUEST_DEPOSIT") {
        const results = await Promise.all([
          ...
          supabase
            ...
            .insert({
              user_id: currentUser.id,
              message: `Your deposit request for ${payload.amount} VTX is pending.`,
              type: "DEPOSIT_PENDING",
              created_at: Date.now(),
            }),
        ]);
        ...
        await notifyAdmins(
          `@${currentUser.username} requested a local deposit of ${payload.amount} VTX.`,
          "DEPOSIT_PENDING",
        );
        ...
        const { data: userData, error: userLookupError } = await supabase
          ...
          .single();
        if (userLookupError || !userData)
          throw new Error("Failed to verify balance details.");
        ...
        if (payload.amount < minAmount || payload.amount > maxAmount) {
          throw new Error(
            `Withdrawal must be between ${minAmount} and ${maxAmount} VTX Coins`,
          );
        }
        ...
        const { error: deductionError } = await supabase.rpc(
          "increment_balance",
          { row_id: currentUser.id, amount: -payload.amount },
        );
        if (deductionError)
          throw new Error(
            "Failed to deduct withdrawal funds: " + deductionError.message,
          );
        ...
        const results = await Promise.all([
          supabase.from("pending_requests").insert({
            ...
            user_id: currentUser.id,
            amount: payload.amount,
            easypaisa_number: payload.easypaisaNumber,
            status: "PENDING",
            date: Date.now(),
          }),
          supabase
            ...
            .insert({
              user_id: currentUser.id,
              message: `Your withdrawal request for ${payload.amount} VTX has been submitted.`,
        ...
        await notifyAdmins(
          `@${currentUser.username} requested a withdrawal of ${payload.amount} VTX (${payload.easypaisaNumber}).`,
          "WITHDRAWAL_PENDING",
        );
        ...
        <div className="h-[100dvh] w-full flex flex-col items-center justify-center bg-[#050505] p-6 text-center text-white select-none relative overflow-hidden font-sans">
          ...
          <div className="relative z-10 flex flex-col items-center gap-6 animate-in fade-in zoom-in duration-500">
            ...
            <div className="space-y-1.5">
              <p className="text-sm font-bold text-gray-300 uppercase tracking-widest animate-pulse">
                Loading Votex...
              </p>
              <p className="text-[10px] text-gray-500 font-medium">
                Connecting to competitive network
              </p>
            </div>
          </div>
        </div>
